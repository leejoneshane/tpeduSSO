...
class v2_schoolController extends Controller
{
    public function all(Request $request)
    {
        ...
        $sid = $request->get('sid');
        if ($sid) {
            $condition[] = "(tpUniformNumbers=$sid)";
        }
        ...
        if (is_array($teachers)) {
            foreach ($teachers as $teacher) {
                ...
                foreach ($classes as $class) {
                    $a = explode(',', $class);
                    if (count($a) == 3 && $a[0] == $dc && $a[2] == $subj_id && !in_array($a[1], $json)) {
                        $json[] = $a[1];
                    }
                    if (count($a) == 2 && $a[1] == $subj_id && !in_array($a[0], $json)) {
                        $json[] = $a[0];
                    }
                }
            }
    ...
    public function allSubjectsByClass(Request $request, $dc, $class_id)
    {
        ...
        if (!empty($teachers)) {
            foreach ($teachers as $teacher) {
                ...
                foreach ($classes as $class) {
                    $a = explode(',', $class);
                    if (count($a) == 3 && $a[0] == $dc && $a[1] == $class_id && !in_array($a[2], $json)) {
                        $json[] = $a[2];
                    }
                    if (count($a) == 2 && $a[0] == $class_id && !in_array($a[1], $json)) {
                        $json[] = $a[1];
                    }
                }
            }
    ...
    public function peopleAdd(Request $request, $dc)
    {
        ...
        if (empty($idno)) {
            return response()->json(['error' => '請提供身分證字號'], 400, array(JSON_UNESCAPED_UNICODE));
        }
        ...
        if ($request->get('type') != '學生') {
            ...
            if (empty($request->get('stdno'))) {
                return response()->json(['error' => '請提供學號'], 400, array(JSON_UNESCAPED_UNICODE));
            }
            $info['employeeNumber'] = $request->get('stdno');
            if (empty($request->get('class'))) {
                return response()->json(['error' => '請提供就讀班級'], 400, array(JSON_UNESCAPED_UNICODE));
            }
            $info['tpClass'] = $request->get('class');
            if (empty($request->get('seat'))) {
                return response()->json(['error' => '請提供學生座號'], 400, array(JSON_UNESCAPED_UNICODE));
            }
    ...
    public function people(Request $request, $dc, $uuid)
    {
        ...
        if ($user && $user->is_parent) {
            ...
            if ($person['employeeType'] == '學生') {
                $kids = PSLink::where('student_idno', $person['cn'])->where('verified', 1)->get();
                foreach ($kids as $kid) {
                    $idno = $kid->parent_idno;
                    $uuid = $openldap->getUserUUID($idno);
                    if (!$uuid) {
                ...
                $kids = PSLink::where('parent_idno', $person['cn'])->where('verified', 1)->get();
                foreach ($kids as $kid) {
                    $idno = $kid->student_idno;
                    $uuid = $openldap->getUserUUID($idno);
                    if ($uuid) {
    ...
    public function listOrgs(Request $request, $area = '')
    {
        ...
        if (is_array($data)) {
            foreach ($data as $org) {
                if (empty($area) || isset($org->st) && $area == $org->st) {
                    $orgs[] = $org;
                }
            }
        }
    ...
    public function listClasses(Request $request, $dc, $grade = '')
    {
        ...
        foreach ($data as $class) {
            if (empty($grade) || $grade == substr($class->ou, 0, 1)) {
                $classes[] = $class;
            }
        }
        ...
        foreach ($teachers as $one) {
            ...
            if (isset($one['titleName'][$dc])) {
                foreach ($one['titleName'][$dc] as $role) {
                    $a = explode(',', $role->key);
                    if (count($a) == 3 && $a[0] == $dc && $a[1] == $ou) {
                        $teacher->title = $role->name;
                    }
                }
                if (!isset($teacher->title)) {
                    $teacher->title = $one['titleName'][$dc][0]->name;
                }
            }
