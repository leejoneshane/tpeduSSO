...
class v2_schoolController extends Controller
{
    public function all(Request $request)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '找不到符合條件的機關學校'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function one(Request $request, $dc)
    {
        ...
        if (!$entry) return response()->json([ 'error' => '找不到指定的機關學校'], 404, array(JSON_UNESCAPED_UNICODE));
		$json = $openldap->getOrgData($entry);
        unset($json['tpAdministrator']);
        if ($json)
            return response()->json($json, 200, array(JSON_UNESCAPED_UNICODE));
        else
            return response()->json([ 'error' => '找不到指定的機關學校'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allTeachersByOrg(Request $request, $dc)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該機關學校查無教職員'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function peopleSearch(Request $request, $dc)
    {
        ...
        if (count($condition) > 1) {
            ...
            if ($json)
                ...
                return response()->json([ 'error' => "找不到符合條件的人員"], 404, array(JSON_UNESCAPED_UNICODE));
        } else {
            return response()->json([ 'error' => '請提供搜尋條件'], 500, array(JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    public function allOu(Request $request, $dc)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該機關學校查無行政部門'], 404, array(JSON_UNESCAPED_UNICODE));
   }
    ...
    public function oneOu(Request $request, $dc, $ou_id)
    {
        ...
        if (!$entry) return response()->json([ 'error' => '找不到指定的行政部門'], 404, array(JSON_UNESCAPED_UNICODE));
		$json = $openldap->getOuData($entry);
        if ($json)
            return response()->json($json, 200, array(JSON_UNESCAPED_UNICODE));
        else
            return response()->json([ 'error' => '找不到指定的行政部門'], 404, array(JSON_UNESCAPED_UNICODE));
    }
	...
	if (is_array($teachers))
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該行政部門查無在職人員'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allSubject(Request $request, $dc)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該機關學校查無教學科目'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function oneSubject(Request $request, $dc, $subj_id)
    {
        ...
        if (!$entry) return response()->json([ 'error' => '找不到指定的教學科目'], 404, array(JSON_UNESCAPED_UNICODE));
		$json = $openldap->getSubjectData($entry);
        if ($json)
            return response()->json($json, 200, array(JSON_UNESCAPED_UNICODE));
        else
            return response()->json([ 'error' => '找不到指定的教學科目'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allTeachersBySubject(Request $request, $dc, $subj_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該教學科目查無任教教師'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allClassesBySubject(Request $request, $dc, $subj_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該教學科目查無授課班級'], 404, array(JSON_UNESCAPED_UNICODE));
   }
    ...
    public function allRole(Request $request, $dc, $ou_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該行政部門查無職務資訊'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function oneRole(Request $request, $dc, $ou_id, $role_id)
    {
        ...
        if (!$entry) return response()->json([ 'error' => '找不到指定的行政職務'], 404, array(JSON_UNESCAPED_UNICODE));
		$json = $openldap->getRoleData($entry);
        if ($json)
            return response()->json($json, 200, array(JSON_UNESCAPED_UNICODE));
        else
            return response()->json([ 'error' => '找不到指定的行政職務'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allTeachersByRole(Request $request, $dc, $ou_id, $role_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該行政職務查無在職人員'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allClass(Request $request, $dc)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該機關學校查無班級'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function oneClass(Request $request, $dc, $class_id)
    {
        ...
        if (!$entry) return response()->json([ 'error' => '找不到指定的班級'], 404, array(JSON_UNESCAPED_UNICODE));
		$json = $openldap->getOuData($entry);
        if ($json)
            return response()->json($json, 200, array(JSON_UNESCAPED_UNICODE));
        else
            return response()->json([ 'error' => '找不到指定的班級'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allTeachersByClass(Request $request, $dc, $class_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該班級查無任教老師'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function allSubjectsByClass(Request $request, $dc, $class_id)
    {
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '該班級查無教學科目'], 404, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function peopleAdd(Request $request, $dc)
    {
        ...
        if (empty($idno)) return response()->json(["error" => "請提供身分證字號"], 400, array(JSON_UNESCAPED_UNICODE));
        $entry = $openldap->getUserEntry($idno);
        if ($entry) return response()->json(["error" => "該使用者已經存在"], 400, array(JSON_UNESCAPED_UNICODE));
        ...
        if ($request->get('type') != "學生") {
            ...
            if (empty($request->get('stdno'))) return response()->json(["error" => "請提供學號"], 400, array(JSON_UNESCAPED_UNICODE));
            $info['employeeNumber'] = $request->get('stdno');
            if (empty($request->get('class'))) return response()->json(["error" => "請提供就讀班級"], 400, array(JSON_UNESCAPED_UNICODE));
            $info['tpClass'] = $request->get('class');
            if (empty($request->get('seat'))) return response()->json(["error" => "請提供學生座號"], 400, array(JSON_UNESCAPED_UNICODE));
        ...
        if ($json)
            ...
            return response()->json([ 'error' => '人員新增失敗：' . $openldap->error() ], 404, array(JSON_UNESCAPED_UNICODE));
    }
...
//		$result = $openldap->deleteEntry($entry);
		if ($result)
		    return response()->json([ 'success' => '指定的人員已經刪除'], 410, array(JSON_UNESCAPED_UNICODE));
		else
		    return response()->json([ 'error' => '指定的人員刪除失敗：' . $openldap->error() ], 500, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function listOrgs(Request $request, $area = '')
    {
		...
		return response()->json($orgs, 200, array(JSON_UNESCAPED_UNICODE));
    }
    ...
    public function listClasses(Request $request, $dc, $grade = '')
    {
		...
		return response()->json($classes, 200, array(JSON_UNESCAPED_UNICODE));
    }
