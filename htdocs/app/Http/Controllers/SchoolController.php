...
class SchoolController extends Controller
{
    ...
    public function index($dc)
    {
        ...
        $sims = '';
        if (array_key_exists('tpSims', $data)) {
            $sims = $data['tpSims'];
        }
    ...
    public function schoolStudentSearchForm(Request $request, $dc)
    {
        ...
        if ($data) {
            $my_ou = $data[0]->ou;
            foreach ($data as $ou) {
                if (!array_key_exists($ou->ou, $ous)) {
                    $ous[$ou->ou] = $ou->description;
                }
            }
        }
        $my_field = $request->get('field');
        if (empty($my_field) && isset($my_ou)) {
            $my_field = "ou=$my_ou";
        }
    ...
    public function importSchoolStudent(Request $request, $dc)
    {
        ...
        if ($request->hasFile('json')) {
            ...
            foreach ($students as $person) {
                ...
                if (isset($person->register) && !empty($person->register)) {
                    $entry['registeredAddress'] = self::chomp_address($person->register);
                }
                if (isset($person->address) && !empty($person->address)) {
                    $entry['homePostalAddress'] = self::chomp_address($person->address);
                }
                if (isset($person->www) && !empty($person->www)) {
                    $entry['wWWHomePage'] = $person->www;
                }
        ...
        if ($request->hasFile('json')) {
            ...
            foreach ($teachers as $person) {
                ...
                if (isset($person->role)) {
                    ...
                    foreach ($titles as $title_pair) {
                        if (!in_array("$dc,$title_pair", $roles)) {
                            $roles[] = "$dc,$title_pair";
                        }
                    }
                }
                ...
                if (isset($person->tclass)) {
                    ...
                    foreach ($classes as $class_pair) {
                        ...
                        if ($openldap->getOuEntry($dc, $a[0])) {
                            $assign[] = "$dc,$class_pair";
                        }
                    }
                }
                $info = array();
                ...
                if (isset($person->register) && !empty($person->register)) {
                    $info['registeredAddress'] = self::chomp_address($person->register);
                }
                if (isset($person->address) && !empty($person->address)) {
                    $info['homePostalAddress'] = self::chomp_address($person->address);
                }
                if (isset($person->www) && !empty($person->www)) {
                    $info['wWWHomePage'] = $person->www;
                }
    ...
    public function schoolTeacherEditForm(Request $request, $dc, $uuid = null)
    {
        ...
        if ($data) {
            foreach ($data as $class) {
                if (!array_key_exists($class->ou, $classes)) {
                    $classes[$class->ou] = $class->description;
                }
            }
        }
        ...
        if ($data) {
            foreach ($data as $role) {
                if (!array_key_exists($role->cn, $roles)) {
                    $roles[$role->cn] = $role->description;
                }
            }
        }
        $school = $openldap->getOrgEntry($dc);
        ...
        if ($original['employeeType'] != $info['employeeType']) {
            ...
            if (!empty($original['info'])) {
                ...
                foreach ($educloud as $k => $c) {
                    ...
                    if (array_key_exists('sid', $i) && $i['sid'] == $sid) {
                        unset($educloud[$k]);
                    }
                }
            }
        ...
        if ($result) {
            if ($original['cn'] != $idno) {
                ...
                if ($result) {
                    ...
                    if ($request->user()->idno == $original['cn']) {
                        Auth::logout();
                    }
        ...
        if ($role != $info['cn']) {
            ...
            foreach ($users as $user) {
                ...
                foreach ($titles as $title) {
                    $a = explode(',', $title);
                    if (count($a) == 3 && ($a[0] != $dc || $a[1] != $ou || $a[2] != $role)) {
                        $roles[] = $title;
                    }
                    if (count($a) == 1 && $a[0] != $role) {
                        $roles[] = $dc.','.$ou.','.$title;
                    }
                }
                ...
                foreach ($users as $user) {
                    ...
                    if (!empty($cid)) {
                        ...
                        if ($cid && !in_array($cid, $all_classes)) {
                            $all_classes[] = $class;
                            if (!in_array($class->grade, $grades)) {
                                $grades[] = $class->grade;
                            }
    ...
    public function schoolClassAssignForm(Request $request, $dc)
    {
        $my_grade = $request->get('grade', 1);
        $my_ou = $request->get('ou', '');
        if ($request->session()->has('grade')) {
            $my_grade = $request->session()->get('grade');
        }
        if ($request->session()->has('ou')) {
            $my_ou = $request->session()->get('ou');
        }
        $openldap = new LdapServiceProvider();
    ...
    public function assignSchoolClass(Request $request, $dc)
    {
        ...
        foreach ($teachers as $teacher) {
            ...
            if ($act == 'add') {
                ...
                if (!$result) {
                    $erros[] = $tname.'：取代配課資訊失敗！';
                }
            } elseif ($act == 'del') {
                $result = $openldap->deleteData($entry, $info);
                if (!$result) {
                    $erros[] = $tname.'：移除配課資訊失敗！';
                }
            }
        }
        if (count($errors) > 0) {
    ...
    public function createSchoolClass(Request $request, $dc)
    {
        ...
        if (!empty($idno)) {
            ...
            if ($teacher) {
                $openldap->updateData($teacher, ['tpTutorClass' => $class]);
            }
        }
        $info = array();
        $info['objectClass'] = array('organizationalUnit', 'top');
    ...
    public function updateSchoolClass(Request $request, $dc, $class)
    {
        ...
        if (!empty($idno)) {
            ...
            if ($org_teacher) {
                ...
                if ($teacher_entry) {
                    $openldap->deleteData($teacher_entry, ['tpTutorClass' => []]);
                }
            }
            $teacher = $openldap->getUserEntry($idno);
            if ($teacher) {
    ...
    public function updateSchoolProfile(Request $request, $dc)
    {
        ...
        $info['wWWHomePage'] = [];
        if (!empty($request->get('wWWHomePage'))) {
            $info['wWWHomePage'] = $request->get('wWWHomePage');
        }
        $info['tpUniformNumbers'] = strtoupper($request->get('tpUniformNumbers'));
        $info['tpIpv4'] = [];
        if (!empty($request->get('tpIpv4'))) {
            $info['tpIpv4'] = $request->get('tpIpv4');
        }
        $info['tpIpv6'] = [];
        if (!empty($request->get('tpIpv6'))) {
