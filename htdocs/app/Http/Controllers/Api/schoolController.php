...
class schoolController extends Controller
{
    public function all()
    {
        ...
        if ($json) {
            ...
            return response()->json(['error' => '找不到任何機關學校'], 404, array(JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    public function peopleAdd(Request $request, $dc)
    {
        ...
        if ($request->get('type') != '學生') {
            ...
            if (empty($request->get('stdno'))) {
                return response()->json(['error' => "please provide user's Student Number!"], 400, array(JSON_UNESCAPED_UNICODE));
            }
            $info['employeeNumber'] = $request->get('stdno');
            if (empty($request->get('class'))) {
                return response()->json(['error' => "please provide user's Study Class!"], 400, array(JSON_UNESCAPED_UNICODE));
            }
            $info['tpClass'] = $request->get('class');
            if (empty($request->get('seat'))) {
                return response()->json(['error' => "please provide user's Classroom Seat Number!"], 400, array(JSON_UNESCAPED_UNICODE));
            }
        ...
        if ($json) {
            ...
            return response()->json(['error' => '人員新增失敗'], 404, array(JSON_UNESCAPED_UNICODE));
        }
    }
...
//      $result = $openldap->deleteEntry($entry);
        if ($result) {
            ...
            return response()->json(['error' => '指定的人員刪除失敗'], 500, array(JSON_UNESCAPED_UNICODE));
        }
    }
