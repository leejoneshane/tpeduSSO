...
class BureauController extends Controller
{
    ...
    public function listProjects(Request $request)
	{
		...
		if ($projects->isEmpty()) {
			$clients = Passport::client()->where('personal_access_client', 0)->where('password_client', 0)->latest()->get();
			if ($clients) {
    ...
    public function storeProject(Request $request)
	{
		...
		if ($request->get('uuid')) {
			...
			$project->forceFill([
				...
				'privileged' => $request->has('privileged') ? true : false,
	...
	{
		$projects = Project::whereNotNull('client')->latest()->get();
		if ($projects->isEmpty()) {
			$clients = Passport::client()->where('personal_access_client', 0)->where('password_client', 0)->latest()->get();
			if ($clients) {
				foreach ($clients as $client) {
    ...
    public function createBureauOrg(Request $request)
    {
		...
		$validatedData = $request->validate([
			...
			'postalCode' => 'nullable|digits_between:3,6',
