...
class GoogleServiceProvider extends ServiceProvider
{
    ...
    {
        if (!strpos($userKey, '@')) {
            $userKey .= '@'.config('saml.email_domain');
        }
        try {
    ...
    {
        try {
            return $this->directory->groups->listGroups(array('domain' => config('saml.email_domain')))->getGroups();
        } catch (\Google_Service_Exception $e) {
            if (config('google.debug')) {
                Log::debug('Google Service Caught exception: '.$e->getMessage()."\n");
            }
    ...
    public function createCourse($name, $ownerId, $section, $descriptionHeading, $description, $room, $courseState)
    {
        ...
        if (!empty($descriptionHeading)) {
            $course->setDescriptionHeading($descriptionHeading);
        }
