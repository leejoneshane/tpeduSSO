			...
			<form role="form" method="POST" action="{{ isset($user) ? route('school.updateTeacher', [ 'uuid' => $user['entryUUID'] ]) : route('school.createTeacher') }}">
			    ...
			    <div class="form-group">
					...
					@foreach ($ous as $ou => $desc)
			    		<option value="{{ $ou }}" {{ isset($user) && $ou == $user['ou'] ? 'selected' : '' }}>{{ $desc }}</option>
			    	@endforeach
					</select>
				</div>
			    <div class="form-group">
					...
					@foreach ($roles as $role => $desc)
			    		<option value="{{ $role }}" {{ isset($user) && $role == $user['title'] ? 'selected' : '' }}>{{ $desc }}</option>
			    	@endforeach
					</select>
				</div>
			    <div class="form-group{{ $errors->has('tclass') ? ' has-error' : '' }}">
					<label>任教班級</label>
					<input id="tclass" type="text" class="form-control" name="tclass" value="{{ isset($user) && array_key_exists('tpTeachClass', $user) ? $user['tpTeachClass'] : '' }}" placeholder="若任教多班，請在班級與班級之間使用半形空白加以區隔...">
