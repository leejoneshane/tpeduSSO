...
<div class="container">
	...
	<div class="col-sm-8">
		...
		<div class="panel-body">
			<form role="form" method="POST" action="{{ route('tutor.updateStudent', [ 'dc' => $dc, 'ou' => $ou, 'uuid' => $user['entryUUID'] ]) }}">
		    	@csrf
				<input type="hidden" name="uuid" value="{{ $user['entryUUID'] }}">
			    <div class="form-group{{ $errors->has('idno') ? ' has-error' : '' }}">
					<label>身分證字號</label>
					<input id="idno" type="text" class="form-control" name="idno" value="{{ $user['cn'] }}" required readonly>
					@if ($errors->has('idno'))
						<p class="help-block">
			    ...
			    <div class="form-group{{ $errors->has('stdno') ? ' has-error' : '' }}">
					<label>學號</label>
					<input id="stdno" type="text" class="form-control" name="stdno" value="{{ array_key_exists('employeeNumber', $user) ? $user['employeeNumber'] : '' }}" required>
					@if ($errors->has('stdno'))
						<p class="help-block">
			    ...
			    <div class="form-group{{ $errors->has('seat') ? ' has-error' : '' }}">
					<label>座號</label>
					<input id="seat" type="text" class="form-control" name="seat" value="{{ array_key_exists('tpSeat', $user) ? $user['tpSeat'] : '' }}" required>
					@if ($errors->has('seat'))
						<p class="help-block">
			    ...
			    <div class="form-group{{ $errors->has('birth') ? ' has-error' : '' }}">
					<label>出生日期</label>
					<input id="birth" type="date" class="form-control" name="birth" value="{{ substr($user['birthDate'],0,4).'-'.substr($user['birthDate'],4,2).'-'.substr($user['birthDate'],6,2) }}">
					@if ($errors->has('birth'))
						<p class="help-block">
			    ...
			    <div class="form-group{{ $errors->has('mail') ? ' has-error' : '' }}">
					...
					@if (array_key_exists('mail', $user))
						@if (is_array($user['mail']))
							...
							<input type="text" class="form-control" style="width:50%;display:inline" name="mail[]" value="{{ $mail }}" placeholder="請勿填寫家長電子郵件" required><button type="button" class="btn btn-danger btn-circle" onclick="$(this).prev().remove();$(this).remove();"><i class="fa fa-minus"></i></button>
							@endforeach
						@else
							<input type="text" class="form-control" style="width:50%;display:inline" name="mail[]" value="{{ $user['mail'] }}" placeholder="請勿填寫家長電子郵件" required><button type="button" class="btn btn-danger btn-circle" onclick="$(this).prev().remove();$(this).remove();"><i class="fa fa-minus"></i></button>
						@endif
					@endif
