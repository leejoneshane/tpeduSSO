	...
	<div class="col-sm-8" style="margin-left:18%">
		...
		<div class="panel-body">
			...
			@endif
				...
				<input type="text" class="form-control" name="organization" value="{{ isset($project->organization) ?: '' }}" required>
			    ...
			    <div class="form-group{{ $errors->has('applicationName') ? ' has-error' : '' }}">
					...
					<input type="text" class="form-control" name="applicationName" value="{{ isset($project->applicationName) ?: '' }}" placeholder="用於顯示在授權頁面，讓使用者得知：何種應用平臺透過單一身份驗證服務進行身份認證" required>
			    ...
			    <div class="form-group{{ $errors->has('reason') ? ' has-error' : '' }}">
					...
					<textarea rows="3" cols="40" class="form-control" name="reason" placeholder="就欲申請介接之目的、未來規劃…等，簡述說明。如：「大學申請入學─備審資料數位化系統」，做為學生參加大學甄試所需之學習履歷相關資料" required>{{ isset($project->reason) ?: '' }}</textarea>
			    ...
			    <div class="form-group{{ $errors->has('website') ? ' has-error' : '' }}">
					...
					<input type="text" class="form-control" name="website" value="{{ isset($project->website) ?: '' }}" required>
			    ...
			    <div class="form-group{{ $errors->has('redirect') ? ' has-error' : '' }}">
					...
					<input type="text" class="form-control" name="redirect" value="{{ isset($project->redirect) ?: '' }}" required>
		    	...
		    	<div class="form-group">
				    ...
				    <div class="form-group{{ $errors->has('connName') ? ' has-error' : '' }}">
						...
						<input type="text" class="form-control" name="connName" value="{{ isset($project->connName) ?: '' }}" required>
				    ...
				    <div class="form-group{{ $errors->has('connUnit') ? ' has-error' : '' }}">
						...
						<input type="text" class="form-control" name="connUnit" value="{{ isset($project->connUnit) ?: '' }}">
				    ...
				    <div class="form-group{{ $errors->has('connEmail') ? ' has-error' : '' }}">
						...
						<input type="email" class="form-control" name="connEmail" value="{{ isset($project->connEmail) ?: '' }}" required>
				    ...
				    <div class="form-group{{ $errors->has('connTel') ? ' has-error' : '' }}">
						...
						<input type="text" class="form-control" name="connTel" value="{{ isset($project->connTel) ?: '' }}" pattern="^([0-9]{10}|[0-9]{9}|[0-9]{8}|[0-9]{7})$" required>
			    ...
			    <div class="form-group{{ $errors->has('memo') ? ' has-error' : '' }}">
					...
					<textarea rows="3" cols="40" class="form-control" name="memo">{{ isset($project->memo) ?: '' }}</textarea>
				</div>
			    <div class="form-group">
